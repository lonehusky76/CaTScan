...
for i = 1:loop_length
    ...
    try
       ...
       %invert data for cepheid1 voltage tracings
        %norm_Ca = gnegate(norm_Ca);
        ...
        % We have removed the paceable query, Leona, Xiaoting and Vassilios
        % 6-26-2024
        else
            binary_paceable(i) = 0;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Save the pacing traces
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        TraceFileName = strcat(folder, '/', savename, '_output_trace.xls');
        %csvwrite (TraceFileName, norm_Ca)
        colName = getColumnName(indexxed);
        writematrix(norm_Ca, TraceFileName,'Range',sprintf('%s1', colName));
        indexxed = indexxed+1;
end
